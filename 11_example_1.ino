...
if (dist_raw <= _TARGET_LOW) {
    myservo.writeMicroseconds(_DUTY_MIN); // 최소 각도로 회전
  }
  else if (dist_raw <= _TARGET_HIGH) {
    myservo.writeMicroseconds(10 * (dist_raw - _TARGET_LOW )); // 중간 각도로 회전
  }
  else {
    myservo.writeMicroseconds(_DUTY_MAX); // 최대 각도로 회전
  }
